...
const els = {
  ...
  rules: $("#rules"), backtestSummary: $("#backtestSummary"), topPairs: $("#topPairs"), topNumbers: $("#topNumbers"), drawHistory: $("#drawHistory"), drawSearch: $("#drawSearch"), statusMessage: $("#statusMessage"),
};
...
function scoreStaticTicket(numbers, weights, pairWeights = new Map()) { const sorted = normalizeTicketNumbers(numbers); const base = sorted.reduce((sum, number) => sum + (weights.get(number) || 0.05), 0) / 6; const spreadBonus = Math.min(new Set(sorted.map((n) => Math.floor((n - 1) / 10))).size, 5) * 0.01; let pairScore = 0; let pairCount = 0; for (let i = 0; i < sorted.length; i += 1) { for (let j = i + 1; j < sorted.length; j += 1) { pairScore += pairWeights.get(`${sorted[i]}-${sorted[j]}`) || 0; pairCount += 1; } } return base + spreadBonus + ((pairScore / Math.max(pairCount, 1)) * 0.06); }
...
function buildStaticDashboardVariant(basePayload, params) {
  ...
  const pairMap = new Map((basePayload.strategy?.pair_weights || []).map((item) => [normalizeTicketNumbers(item.numbers).join("-"), Number(item.weight) || 0]));
  ...
  for (let attempt = 0; candidates.length < targetCandidates && attempt < 5000; attempt += 1) {
    ...
    seen.add(key);
    candidates.push({ numbers, score: scoreStaticTicket(numbers, weightMap, pairMap) });
  }
...
function renderStrategy(strategy) { els.rules.innerHTML = strategy.rules.map((r) => `<span class="chip">${r}</span>`).join(""); const backtest = strategy.backtest || {}; els.backtestSummary.innerHTML = backtest.rounds ? `<article><strong>${backtest.rounds}회</strong><span>백테스트</span></article><article><strong>${Number(backtest.avg_best_matches).toFixed(2)}</strong><span>평균 최고 적중</span></article><article><strong>${Math.round(Number(backtest.three_plus_rate) * 100)}%</strong><span>3개 이상</span></article><article><strong>${Math.round(Number(backtest.four_plus_rate) * 100)}%</strong><span>4개 이상</span></article>` : ""; els.topPairs.innerHTML = (strategy.top_pairs || []).map((item) => `<span class="chip">pair ${formatNumbers(item.numbers)} · ${Number(item.weight).toFixed(3)}</span>`).join(""); const max = Math.max(...strategy.top_numbers.map((i) => i.weight), 1); els.topNumbers.innerHTML = strategy.top_numbers.map((i) => `<div class="signal-row"><strong>${String(i.number).padStart(2, "0")}</strong><div class="signal-bar"><span style="width:${Math.round((i.weight / max) * 100)}%"></span></div><small>${Number(i.weight).toFixed(3)}</small></div>`).join(""); }
